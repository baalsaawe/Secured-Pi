...
For accuracy, we recommend a minimum of 15 pictures for each member.  It is
...
def test_individual(image_to_test, threshold=40,
    ...
    train_recognizer function. Returns the prediction and uncertainty measure.
    """
    ...
    member_prediction, uncertainty = recognizer.predict(test_image)
    if verbose is True:
        print('member number: {}, uncertainty: {}'.format(member_prediction,
                                                          uncertainty))
    return (member_prediction, uncertainty)
